...
import pickle
from embeddings_hm import Embeddings

import rembg

from cam_utils import orbit_camera, OrbitCamera
from gs_renderer_drop import Renderer, MiniCam
...
class GUI:
    def __init__(self, opt):
        ...
        self.gaussain_scale_factor = 1  # scale the gaussian for rendering
    ...
    def prepare_train(self):
        ...
        # TODO: Load the local embeddings directly
        ...
        with torch.no_grad():
            ...
            if self.enable_sd:
                if self.opt.imagedream:
                    ...
                    '''
                    # ----------------Save .pkl embedding file------------------------------
                    self.guidance_sd.get_text_embeds([self.prompt], [self.negative_prompt])
                    print("------------This is text embeds--------------")
                    embedding = self.guidance_sd.get_text_embeds([self.prompt], [self.negative_prompt])
                    print("##embedding is(save in ./embeds/) : ", embedding)
                    print("##prompt is: ", self.prompt)
                    print("##neg prompt is: ", self.negative_prompt)
                    with open('./embeds/text_embedding2.pkl', 'wb') as f:
                        pickle.dump(embedding, f)    
                    print("------------End------------------------------")
                    # -----------------END--------------------------------------------------
                    
                    #-----------------Read .pkl embedding file (Back-up)--------------------
                    try:
                        with open('./embeds/text_embedding.pkl', 'rb') as f:
                            self.embeddings = pickle.load(f)
                            print("##embedding is(save in ./embeds/) : ", self.embeddings)
                            print("##pos: ", self.embeddings['pos'])
                            print("Available keys in embeddings:", self.embeddings.keys())
                            print("##embedding is (saved in ./embeds/): ")
                        ...
                        print("Embedding file not found. Please ensure the embeddings are precomputed and saved.")
                    #-------------------END--------------------------------------------------
                    '''

                    # TODO: write the embedding to file, than, load the embeddings from our Sensor CLIP and local file
            if self.enable_zero123:
                self.guidance_zero123.get_img_embeds(self.input_img_torch)

    def train_step(self):
        # print("Current keys in embeddings:", self.embeddings.keys())
        ...
        for _ in range(self.train_steps):
            ...
            if self.input_img_torch is not None and not self.opt.imagedream:
                ...
                out = self.renderer.render(cur_cam, drop_probability=self.opt.drop_probability)

                # rgb loss
                image = out["image"].unsqueeze(0)  # [1, 3, H, W] in [0, 1]
                loss = loss + 10000 * (step_ratio if self.opt.warmup_rgb_loss else 1) * F.mse_loss(image,
                                                                                                   self.input_img_torch)

                # mask loss
                mask = out["alpha"].unsqueeze(0)  # [1, 1, H, W] in [0, 1]
                loss = loss + 1000 * (step_ratio if self.opt.warmup_rgb_loss else 1) * F.mse_loss(mask,
            ...
            for _ in range(self.opt.batch_size):
                ...
                out = self.renderer.render(cur_cam, bg_color=bg_color, drop_probability=self.opt.drop_probability)
                # print the shape of out["visibility_filter"]
                image = out["image"].unsqueeze(0)  # [1, 3, H, W] in [0, 1]
                images.append(image)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--drop_probability", type=float, default=0.0,
                        help="Probability of dropping Gaussian points during rendering (between 0 and 1)")
    args, extras = parser.parse_known_args()

    # Load the config from the yaml file
    config = OmegaConf.load(args.config)

    # Add drop_probability to the config
    config.drop_probability = args.drop_probability

    # Override default config from CLI
    opt = OmegaConf.merge(config, OmegaConf.from_cli(extras))
